...
const Profile = () => {
  ...
  return (
    <SafeAreaView edges={['top','bottom']} style={[ProfileStyle.safe, { backgroundColor: colors.background }]}>
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={ProfileStyle.flex}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={ProfileStyle.scroll}>

          {/* Hero header */}
          <LinearGradient colors={['#0F172A', '#1E3A5F']} style={ProfileStyle.hero}>
            <View style={[ProfileStyle.avatar, { backgroundColor: colors.primary }]}>
              <Text style={ProfileStyle.avatarText}>{getInitials(user?.full_name ?? 'U')}</Text>
            </View>
            <Text style={ProfileStyle.heroName}>{user?.full_name}</Text>
            <Text style={ProfileStyle.heroEmail}>{user?.email}</Text>
            <View style={ProfileStyle.rolePill}>
              <MaterialIcons name="verified-user" size={12} color="#fff" />
              <Text style={ProfileStyle.roleText}>Customer</Text>
            </View>
          </LinearGradient>
          ...
          {/* Personal info */}
          <View style={[ProfileStyle.section, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <View style={ProfileStyle.sectionHeader}>
              <Text style={[ProfileStyle.sectionTitle, { color: colors.text }]}>Personal Info</Text>
              {editing ? (
                <View style={ProfileStyle.editActions}>
                  <TouchableOpacity onPress={() => { setEditing(false); setFullName(user?.full_name ?? ''); setPhone(user?.phone ?? ''); }}>
                    <Text style={[ProfileStyle.cancelEditText, { color: colors.textMuted }]}>Cancel</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[ProfileStyle.saveBtn, { backgroundColor: colors.primary }]}
                    onPress={handleSave}
                    disabled={saving}
                  >
                    {saving ? <ActivityIndicator size="small" color="#fff" /> : (
                      <Text style={ProfileStyle.saveBtnText}>Save</Text>
                    )}
                  </TouchableOpacity>
                </View>
              ) : (
                <TouchableOpacity
                  style={[ProfileStyle.editBtn, { backgroundColor: colors.primaryLight }]}
                  onPress={() => setEditing(true)}
                >
                  <MaterialIcons name="edit" size={14} color={colors.primary} />
                  <Text style={[ProfileStyle.editBtnText, { color: colors.primary }]}>Edit</Text>
                </TouchableOpacity>
              )}
            </View>
            ...
            {[
              ...
              <View key={f.key} style={ProfileStyle.fieldGroup}>
                <Text style={[ProfileStyle.fieldLabel, { color: colors.textMuted }]}>{f.label}</Text>
                <View style={[ProfileStyle.inputRow, {
                  ...
                  <TextInput
                    style={[ProfileStyle.fieldInput, { color: colors.text }]}
                    value={f.value}
                ...
                {errors[f.key] ? <Text style={[ProfileStyle.error, { color: colors.danger }]}>{errors[f.key]}</Text> : null}
              </View>
            ))}

            {/* Read-only email */}
            <View style={ProfileStyle.fieldGroup}>
              <Text style={[ProfileStyle.fieldLabel, { color: colors.textMuted }]}>Email</Text>
              <View style={[ProfileStyle.inputRow, { borderWidth: 0 }]}>
                <MaterialIcons name="email" size={16} color={colors.textMuted} />
                <Text style={[ProfileStyle.fieldInput, { color: colors.textMuted }]}>{user?.email}</Text>
              </View>
            </View>
          </View>
          ...
          {/* Settings */}
          <View style={[ProfileStyle.section, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <Text style={[ProfileStyle.sectionTitle, { color: colors.text }]}>Settings</Text>
            {[
              {
                icon: 'lock' as keyof typeof MaterialIcons.glyphMap,
              ...
              <TouchableOpacity
                key={item.label}
                style={[ProfileStyle.menuItem, i < arr.length - 1 && { borderBottomWidth: 1, borderBottomColor: colors.border }]}
                onPress={item.action}
                activeOpacity={0.7}
              >
                <MaterialIcons name={item.icon} size={20} color={item.color} />
                <Text style={[ProfileStyle.menuLabel, { color: colors.text }]}>{item.label}</Text>
          ...
          <TouchableOpacity
            style={[ProfileStyle.logoutBtn, { backgroundColor: colors.danger, marginHorizontal: 16, marginTop: 8, marginBottom: 16 }]}
            onPress={handleSignOut}
            activeOpacity={0.85}
          >
            <MaterialIcons name="logout" size={20} color="#FFFFFF" />
            <Text style={ProfileStyle.logoutText}>Sign Out</Text>
          </TouchableOpacity>
        </ScrollView>
